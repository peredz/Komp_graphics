...
def main():
    ...
    pygame.display.set_caption("Task C — Landscape smoothing")
    ...
    sub_hm_orig = HeightMap()
    sub_hm_orig.W, sub_hm_orig.H = hm.W // step, hm.H // step
    sub_hm_orig.data = np.array([hm.get(x * step, y * step) 
                                 for y in range(sub_hm_orig.H) 
                                 for x in range(sub_hm_orig.W)], dtype=np.float32)
    ...
    def rebuild_mesh():
        sub_hm = HeightMap()
        sub_hm.W, sub_hm.H = sub_hm_orig.W, sub_hm_orig.H
        sub_hm.data = sub_hm_orig.data.copy()
        
        mesh = Mesh()
        mesh.build(sub_hm, height_scale, height_exp, smoothed, True, 1.0)
        return mesh
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1:
                    ...
                    height_scale = max(1.0, height_scale - 2.0)
                    mesh = rebuild_mesh()
                elif event.key == pygame.K_9:
                    height_exp = max(0.2, height_exp - 0.1)
                    mesh = rebuild_mesh()
        ...
        text1 = font.render(f"Режим: {mode} | Масштаб: {int(height_scale)} | Экспонента: {height_exp:.1f}", True, (220, 220, 220))
        text2 = font.render(f"Сглаживание: {'ВКЛ' if smoothed else 'ВЫКЛ'} | S-toggle H(+/-) E(9/0)", True, (220, 220, 220))
