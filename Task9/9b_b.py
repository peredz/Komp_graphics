...
def triangle_warp_grid(img, grid_size=4):
    h, w = img.shape[:2]

    xs = np.linspace(0, w, grid_size)
    ys = np.linspace(0, h, grid_size)
    ...
    cv2.namedWindow("9B-B 4x4")
    cv2.setMouseCallback("9B-B 4x4", mouse)
    ...
    while True:
        ...
        for p in dst_points:
            cv2.circle(display, tuple(p.astype(int)), 4, (255, 0, 0), -1)

        cv2.imshow("9B-B 4x4", display)
